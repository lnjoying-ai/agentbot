...
const conversations = reactive<Map<string, AgentConversation>>(new Map());
const agentSkills = reactive<Map<string, AgentSkillsResponse>>(new Map());
...
export function useAgentStore() {
  ...
  const currentConversation = computed(() => 
    conversations.get(currentAgentId.value) || {
  ...
  async function fetchAgentSkills(agentId: string) {
    try {
      ...
      agentSkills.set(agentId, data as AgentSkillsResponse);
    } catch (e: any) {
      console.error('Error fetching agent skills:', e);
    }
  }

  async function updateAgentSkills(agentId: string, payload: Partial<AgentSkillsResponse> & { entries?: Record<string, SkillEntryConfig> }) {
    try {
  ...
  async function installSkill(agentId: string, name: string, installId?: string, timeout?: number) {
  ...
  async function updateAgentSkillEntry(agentId: string, skillKey: string, patch: SkillEntryConfig) {
    const current = agentSkills.get(agentId);
    const entries = { ...(current?.entries ?? {}) } as Record<string, SkillEntryConfig>;
  ...
  function isLocalUserDuplicate(messages: any[], incoming: any) {
    if (!incoming || incoming.role !== 'user' || !incoming.content) return false;
    const incomingTime = Date.parse(incoming.timestamp || '') || Date.now();
    return messages.some((msg: any) => {
      if (!msg || msg.role !== 'user' || msg.content !== incoming.content || !msg.local) return false;
  ...
  function setMessages(agentId: string, messages: any[]) {
    let conv = conversations.get(agentId);
    if (!conv) {
      conv = {
        agentId,
  ...
  function prependMessages(agentId: string, messages: any[]) {
    ...
    const existingIds = new Set(conv.messages.map((msg: any) => msg.id));
    const existingKeys = new Set(conv.messages.map((msg: any) => `${msg.role}|${msg.content}|${msg.timestamp}`));
  ...
  function appendMessages(agentId: string, messages: any[]) {
    if (!messages || messages.length === 0) return;
    let conv = conversations.get(agentId);
    if (!conv) {
      conv = {
  ...
  function markConversationRead(agentId: string) {
  ...
  async function fetchAgentStatistics(agentId: string) {
    try {
