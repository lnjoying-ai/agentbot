    ...
    try {
        ...
        if (response.ok) {
            ...
            if (source) {
                ...
                const newState = {};
                if (source.llm) {
                    if (source.llm.apiBaseUrl)
                        newState.apiBaseUrl = source.llm.apiBaseUrl;
                    else if (source.llm.baseUrl)
                        newState.apiBaseUrl = source.llm.baseUrl;
                    // 注意：如果 API Key 包含掩码 (****)，则不填充到编辑框，避免误保存掩码字符串
                    if (source.llm.apiKey && !source.llm.apiKey.includes("****"))
                        newState.apiKey = source.llm.apiKey;
                    if (source.llm.model)
                        newState.defaultModel = source.llm.model;
                    if (source.llm.provider)
                        newState.provider = source.llm.provider;
                    if (source.llm.fallbackOrder)
                        newState.fallbackOrder = source.llm.fallbackOrder;
                    if (source.llm.parallelTools !== undefined)
                        newState.parallelTools = source.llm.parallelTools;
                    if (source.llm.maxToolRounds !== undefined)
                        newState.maxToolRounds = source.llm.maxToolRounds;
                    if (source.llm.toolParallelism !== undefined)
                        newState.toolParallelism = source.llm.toolParallelism;
                    const orKey = source.llm.openrouter?.apiKey || source.llm.openrouterKey;
                    if (orKey && !orKey.includes("****"))
                        newState.openrouterKey = orKey;
                    const gKey = source.llm.glm?.apiKey || source.llm.glmKey;
                    if (gKey && !gKey.includes("****"))
                        newState.glmKey = gKey;
                    const kKey = source.llm.kimi?.apiKey || source.llm.kimiKey;
                    if (kKey && !kKey.includes("****"))
                        newState.kimiKey = kKey;
                }
                if (source.channels) {
...
async function saveToServer() {
    ...
    try {
        ...
        if (state.apiKey && !state.apiKey.includes("****"))
            payload.llm.apiKey = state.apiKey;
        if (state.openrouterKey && !state.openrouterKey.includes("****"))
            payload.llm.openrouter.apiKey = state.openrouterKey;
        if (state.glmKey && !state.glmKey.includes("****"))
            payload.llm.glm.apiKey = state.glmKey;
        if (state.kimiKey && !state.kimiKey.includes("****"))
            payload.llm.kimi.apiKey = state.kimiKey;
        if (state.telegramToken && !state.telegramToken.includes("****"))
            payload.channels.telegram.token = state.telegramToken;
        if (state.wechatWebhook && !state.wechatWebhook.includes("****"))
            payload.channels.wechat.token = state.wechatWebhook;
        const response = await fetch(`${baseUrl}/api/config`, {
            method: "POST",
